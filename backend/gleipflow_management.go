...
// buildHTTPRequestDump creates an HTTP request dump string from a PhantomRequest
func buildHTTPRequestDump(phantom PhantomRequest) string {
	var dump strings.Builder

	// Request line
	dump.WriteString(fmt.Sprintf("%s %s HTTP/1.1\r\n", phantom.Method, phantom.Path))

	// Headers
	for key, value := range phantom.Headers {
		dump.WriteString(fmt.Sprintf("%s: %s\r\n", key, value))
	}

	// Add Content-Length if there's a body
	if phantom.Body != "" {
		dump.WriteString(fmt.Sprintf("Content-Length: %d\r\n", len(phantom.Body)))
	}
	...
	// Body
	if phantom.Body != "" {
		dump.WriteString(phantom.Body)
	}

	return dump.String()
}

// extractHostAndTLS extracts host and TLS info from a phantom request URL
func extractHostAndTLS(url string) (string, bool) {
...
func (a *App) GetPhantomRequests(gleipFlowID string, lastRequest interface{}) ([]PhantomRequest, error) {
	// TODO: Implement actual phantom request generation algorithm
	// For now, return placeholder phantom requests

	// Get the flow to understand context
	_, err := a.GetGleipFlow(gleipFlowID)
	if err != nil {
		return nil, fmt.Errorf("failed to get gleipFlow: %v", err)
	}

	// Placeholder phantom requests - these would be generated by analyzing:
	// - The last request's endpoint patterns
	// - Common REST API patterns
	// - Authentication flows
	// - CRUD operations on detected resources
	phantomRequests := []PhantomRequest{
		{
			Method: "GET",
...
func (a *App) AddPhantomRequestToGleipFlow(gleipFlowID string, phantomRequest PhantomRequest) error {
	...
	newStep := GleipFlowStep{
		...
		RequestStep: &RequestStep{
			...
			Request: func() network.HTTPRequest {
				host, tls := extractHostAndTLS(phantomRequest.URL)
				return network.HTTPRequest{
					Host: host,
					TLS:  tls,
					Dump: buildHTTPRequestDump(phantomRequest),
				}
			}(),
			VariableExtracts:         []VariableExtract{},
			RecalculateContentLength: true,
	...
	trackEvent("phantom_requests", "added", map[string]interface{}{
		"flow_id": gleipFlowID,
		"method":  phantomRequest.Method,
		"path":    phantomRequest.Path,
	})
	...
	fmt.Printf("DEBUG: Successfully added phantom request %s %s to flow %s\n",
		phantomRequest.Method, phantomRequest.Path, gleipFlowID)
	return nil
}
