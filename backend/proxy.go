...
func (p *ProxyServer) GetInterceptedRequests() []*network.HTTPTransaction {
	return p.interceptQueue.GetAll()
}
...
func (p *ProxyServer) ModifyInterceptedResponse(id string, rawResponseDump string) error {
	...
	// Parse the raw response to extract status and body for actual browser response
	if req.ActualResponseStatusCode != 0 { // Only update if we have actual response data
		// Parse the modified response from the raw response dump
		responseLines := strings.Split(rawResponseDump, "\r\n")
		if len(responseLines) > 0 {
			// Parse status line (e.g., "HTTP/1.1 200 OK")
			statusLineParts := strings.SplitN(responseLines[0], " ", 3)
			if len(statusLineParts) >= 3 {
				statusCode, err := strconv.Atoi(statusLineParts[1])
				if err == nil {
					req.ActualResponseStatusCode = statusCode
					req.ActualResponseStatus = strings.Join(statusLineParts[1:], " ")
				}
			}
		}

		// Extract response body from raw dump
		responseBody := p.extractResponseBody(rawResponseDump)
		req.ActualResponseBody = []byte(responseBody)

		// Parse headers from raw dump
		headers := make(map[string]string)
		inHeaders := false
		for _, line := range strings.Split(rawResponseDump, "\r\n") {
			line = strings.TrimRight(line, "\r")
			if !inHeaders && strings.HasPrefix(line, "HTTP/") {
				inHeaders = true
				continue
			}
			if inHeaders && line == "" {
		...
		req.ActualResponseHeaders = headers
	}
...
// extractResponseBody extracts the body from a raw response
func (p *ProxyServer) extractResponseBody(body string) string {
...
func (p *ProxyServer) executeForwardedRequest(req *network.HTTPTransaction, method, url, rawRequest string) {
	...
	if interceptedReq, exists := p.interceptQueue.Get(req.ID); exists {
		...
		// Store the actual response data to send back to browser
		interceptedReq.ActualResponseStatus = resp.Status
		interceptedReq.ActualResponseStatusCode = resp.StatusCode
		interceptedReq.ActualResponseHeaders = make(map[string]string)
		for key, values := range resp.Header {
			if len(values) > 0 {
				interceptedReq.ActualResponseHeaders[key] = values[0]
			}
		}
		interceptedReq.ActualResponseBody = respBody
		...
		if interceptedReq.HistoryID != "" {
			...
			updateTx.Response.Dump = string(responseDump)
		...
		if req.InterceptResponse {
			...
			// Auto-forward response immediately - response is ready, signal that the request can continue
			fmt.Printf("Debug: Auto-forwarding response for request %s\n", req.ID)
			close(interceptedReq.Done)
			// The request will be removed from the intercept queue by the normal proxy flow
		}
	}
}
...
func (p *ProxyServer) handleHTTPRequestWithClient(w http.ResponseWriter, r *http.Request, client *http.Client, isHTTPS bool) {
	...
	if p.interceptEnabled {
		...
		// Get the updated intercepted request from the queue (it might have been updated by executeForwardedRequest)
...
func (p *ProxyServer) handleInterceptedResponse(w http.ResponseWriter, intercepted *network.HTTPTransaction) bool {
	hasActualResponse := intercepted.ActualResponseStatusCode != 0

	if hasActualResponse {
		// Use the stored response data instead of making a new request
		fmt.Printf("Debug: Using stored response data for intercepted request %s\n", intercepted.ID)

		// Set response headers
		for key, value := range intercepted.ActualResponseHeaders {
			w.Header().Set(key, value)
		}
		w.WriteHeader(intercepted.ActualResponseStatusCode)
		w.Write(intercepted.ActualResponseBody)
...
// interceptResponse handles response interception
func (p *ProxyServer) interceptResponse(r *http.Request, requestDump []byte, resp *http.Response, responseDump []byte, isHTTPS bool) *network.HTTPTransaction {
	// Create headers map
	headers := make(map[string]string)
	for key, values := range resp.Header {
		headers[key] = values[0]
	...
	// Create interception request with response
	intercepted := &network.HTTPTransaction{
		ID: uuid.New().String(),
		Request: network.HTTPRequest{
			Host: r.Host,
