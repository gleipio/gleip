<script lang="ts">
  ...
  function handleRequestChange(e: CustomEvent) {
    ...
    // Also parse some basic information for the UI, but preserve existing TLS setting
    const { host } = parseRawHttpRequest(updatedRawRequest);
    
    dispatch('update', { 
      request: {
        ...requestStep.request,
        host, 
        // Preserve existing TLS value instead of overriding with parsed value
        tls: requestStep.request.tls,
