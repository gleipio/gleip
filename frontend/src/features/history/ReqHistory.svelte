<script lang="ts">
  ...
  import { GetProxyRequests, GetProxyRequestsAfter, LaunchFirefoxBrowser, IsFirefoxRunning, CheckFirefoxInstallation, GetTransactionDetails, SearchProxyRequests, GetTransactionMetadata, GetTransactionChunk } from '../../../wailsjs/go/backend/App';
  ...
  async function performSearch() {
    ...
    if (!searchQuery.trim()) {
      // If search is empty, load all requests
      ...
      try {
        ...
        const searchResults = await SearchProxyRequests(searchQuery);
        
        // Sort search results by seqNumber (descending)
        filteredRequests = searchResults.sort((a, b) => b.seqNumber - a.seqNumber);
  ...
  async function loadInitialRequests() {
    ...
    try {
      ...
      console.log(`Loaded ${initialRequests.length} initial requests`);
      ...
      filteredRequests = requests; // Initialize filtered requests with all requests
    ...
    try {
      ...
      if (newRequests.length > 0) {
        ...
        if (uniqueNewRequests.length > 0) {
          ...
          if (!searchQuery.trim()) {
            // If no search query, update the filtered list with all requests
            filteredRequests = requests;
          } else {
            // If active search, refresh the search to include new requests
            await performSearch(); // This already handles empty results correctly
          }
  ...
  onMount(() => {
    // Load initial requests when component mounts
    loadInitialRequests();
      ...
      <div class="border-b border-[var(--color-midnight-darker)] bg-[var(--color-midnight-light)]/70 px-4 py-3 flex items-center gap-4 shrink-0">
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
          ...
          <!-- <button 
            ...
            class="px-3 py-1 text-sm rounded bg-gray-700 text-white hover:bg-gray-600"
