<script lang="ts">
  ...
  import { CopyRequestToClipboard, CopyRequestToSelectedFlow } from '../../../../wailsjs/go/backend/App';
  ...
  function resizeCanvas() {
    ...
    // Update header width to match total columns width
    const headerContainer = document.querySelector('.header-container');
    if (headerContainer) {
      const headerContentDiv = headerContainer.querySelector('div > div') as HTMLElement;
      if (headerContentDiv) {
        headerContentDiv.style.width = `${totalColumnsWidth}px`;
      }
    }
  ...
  <div class="h-full flex flex-col relative">
    ...
    <div class="sticky top-0 left-0 shrink-0 h-6 z-10 overflow-hidden header-container">
      <div class="header-scroll-container overflow-x-auto w-full h-full" style="overflow-y: hidden;">
        <div style="width: {totalColumnsWidth}px; min-width: 100%;" class="flex h-full">
    ...
    >
      <div class="relative" style="width: {totalColumnsWidth}px; height: {totalContentHeight}px;">
